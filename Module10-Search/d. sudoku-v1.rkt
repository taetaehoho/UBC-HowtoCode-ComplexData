...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |d. sudoku-v1|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; board -> boolean
;; produce true if board is solved false if not (if board has no empty space)
...
;; board -> listof board
;; produce list of possible boards from board
;; finds first empty square and fills it with [1, 9] and keeps only valid boards
#;
...
;; Board -> Pos
;; produces position of first blank square
...
;; produce 9 boards with blanked filled with natural
...
;; true if board col valid board row valid and board square valid
;; valid means no unit on the board has same value twice
...
;(define (valid-board bd) true)

(define (valid-board bd)
  (local [(define (valid-units? lou)     ; listofunit -> boolean
           (andmap valid-unit? lou))

          (define (valid-unit? u)        ; unit (list of pos) -> boolean
            (no-dups (only-values (read-unit u))))

          (define (read-unit u)          ; unit -> listofval or false
            (map read-pos u))

          (define (read-pos pos)         ; pos -> val 
            (read-square bd pos))

          (define (only-values lov)
            (filter number? lov))
